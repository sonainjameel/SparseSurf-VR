...
class GaussianModel:
    ...
    def __init__(self, sh_degree : int, feat_dim: int=8, ):
    ...
    def create_from_pcd(self, pcd : BasicPointCloud, spatial_lr_scale : float):
        ...
        surf_feats = torch.zeros((fused_point_cloud.shape[0], self.feat_dim)).float().cuda()
    ...
    def training_setup(self, training_args):
        ...
        l = [
            ...
            {'params': [self._features_rest], 'lr': training_args.feature_lr / training_args.sh_rest_lr_divisor, "name": "f_rest"},
    ...
    def load_ply(self, path):
        ...
        # anchor_feat
        surf_feat_names = [p.name for p in plydata.elements[0].properties if p.name.startswith("f_surf_feat")]
        surf_feat_names = sorted(surf_feat_names, key = lambda x: int(x.split('_')[-1]))
        surf_feats = np.zeros((xyz.shape[0], len(surf_feat_names)))
        for idx, attr_name in enumerate(surf_feat_names):
            surf_feats[:, idx] = np.asarray(plydata.elements[0][attr_name]).astype(np.float32)
    ...
    def prune_points(self, mask):
        ...
        self._surf_feat = optimizable_tensors["surf_feat"]
        self._opacity = optimizable_tensors["opacity"]
    ...
    def densification_postfix(self, new_xyz, new_knn_f, new_features_dc, new_features_rest, new_surf_feat, new_opacities, new_scaling, new_rotation):
        d = {"xyz": new_xyz,
        "knn_f": new_knn_f,
        "f_dc": new_features_dc,
        "f_rest": new_features_rest,
        "surf_feat": new_surf_feat,
    ...
    def densify_and_split(self, grads, grad_threshold, grads_abs, grad_abs_threshold, scene_extent, max_radii2D, N=2):
        ...
        new_surf_feat = self._surf_feat[selected_pts_mask].repeat(N,1)
        ...
        self.densification_postfix(new_xyz, new_knn_f, new_features_dc, new_features_rest, new_surf_feat, new_opacity, new_scaling, new_rotation)
    ...
    def densify_and_clone(self, grads, grad_threshold, scene_extent):
        ...
        if selected_pts_mask.sum() > 0:
            ...
            new_surf_feat = self._surf_feat[selected_pts_mask]
            new_opacities = self._opacity[selected_pts_mask]
            new_scaling = self._scaling[selected_pts_mask]
            new_rotation = self._rotation[selected_pts_mask]
            new_knn_f = self._knn_f[selected_pts_mask]

            self.densification_postfix(new_xyz, new_knn_f, new_features_dc, new_features_rest, new_surf_feat, new_opacities, new_scaling, new_rotation)
    ...
    def get_points_depth_in_depth_map(self, fov_camera, depth, points_in_camera_space, scale=1):
        ...
        pts_projections = torch.stack(
                        [points_in_camera_space[:,0] * fov_camera.Fx / (points_in_camera_space[:,2]+1e-8) + fov_camera.Cx,
                         points_in_camera_space[:,1] * fov_camera.Fy / (points_in_camera_space[:,2]+1e-8) + fov_camera.Cy], -1).float()/scale
